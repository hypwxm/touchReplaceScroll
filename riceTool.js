...
DS.$(function() {
    ...
    loadingbar.innerHTML = "<div class='loading'><img src='/Resources/images/loading.gif'></div>";
...
var loadingAndLoadedservices = {
    ...
    loaded: function (tips, callback, time, opacitytime) {
        ...
        DS.hidetime = setTimeout(function () {
            ...
            $loading.innerHTML = "<img src='/Resources/images/loading.gif'>";
    ...
    return function _imgLoad() {
        ...
        var imgCanUse = function () {
            ...
            for (var i = imgScrollIndex; i < imglen; i++) {
                ...
                var eleBouding = img[i].getBoundingClientRect();

                //只加载屏幕范围内的,屏幕往上往下往左往右超出的部分都不会加载。
                if (eleBouding.bottom <= 0 || eleBouding.right <= 0) {
                    continue;
                }


                if (eleBouding.top < baseH) {
                    if(eleBouding.left < baseW) {
...
function rangeContainDouble(min, max) {
    var rand = Math.random();
    return min + Math.round(rand * (max - min));
}

function rangeContainMax(min, max) {
    var rand = Math.random();
    return min + Math.ceil(rand * (max - min));
}

function rangeContainMin(min, max) {
    var rand = Math.random();
